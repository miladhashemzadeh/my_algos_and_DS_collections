...
class Elevator:
    ...
    def make_action(self, action_number) -> tuple:
        if action_number == self.ACTION_DOWN:
            ...
            if self._current_position <= 0 and direction_verify == self.OUT_OF_LIMIT_DIRECTION:
                return -20, False
            elif self._current_position - 1 > self._floor_size:
                return -20, False
            ...
            self._current_state = self.ACTION_UP
            self._is_waiting = False
            direction_verify = self._make_distance(self._floor_to_getout, self._current_position, 1)
            if self._current_position <= 0:
                return -20, False
            elif self._current_position > self._floor_size and direction_verify == self.OUT_OF_LIMIT_DIRECTION:
                return -20, False
            elif direction_verify == self.OUT_OF_LIMIT_DIRECTION:
                return -20, False
            elif direction_verify == self.GAME_ENDED:
            ...
            self._current_state = self.ACTION_WAIT
    ...
    def _make_distance(self, floor_to_getout, current_position, direction) -> int:
        ...
        try:
            ...
            self._current_state = self.OUT_OF_LIMIT_DIRECTION
            return self.OUT_OF_LIMIT_DIRECTION
        if maxFloor == 0 or nearest == self.GAME_ENDED:
            self._current_state = self.GAME_ENDED
            return self.GAME_ENDED
        else:
            self._current_state = self.RIGHT_DIRECTION if decision else self.WRONG_DIRECTION
            return self.RIGHT_DIRECTION if decision else self.WRONG_DIRECTION

    def _find_nearest(self, floor_to_getout, current_position):
        minimum = 0
        getout_size = floor_to_getout.size - 1
        max_pointer = getout_size if current_position > getout_size else current_position
        min_pointer = getout_size if current_position > getout_size else current_position
        while max_pointer <= getout_size or min_pointer >= minimum:
            if floor_to_getout[max_pointer] != 0:
                return max_pointer
            max_pointer += 1 if max_pointer < getout_size else 0
